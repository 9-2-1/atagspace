...
const appRouter = t.router({
  ...
  tagFileChange: t.procedure
    .input(z.object({ id: z.number(), adds: z.array(z.string()), removes: z.array(z.string()) }))
    .mutation(({ input }) => {
  ...
  // 标签管理
  listCategories: t.procedure.query(() => {
  ...
  addCategory: t.procedure
    .input(z.object({ name: z.string(), color: z.string().optional().nullable() }))
    .mutation(({ input }) => {
  ...
  deleteCategory: t.procedure.input(z.string()).mutation(({ input }) => {
  ...
  deleteTag: t.procedure.input(z.string()).mutation(({ input }) => {
    TagModel.remove(input);
  }),

  // 源文件夹管理
  listSources: t.procedure.query(() => {
  ...
  updateSource: t.procedure
    .input(z.object({ id: z.number(), name: z.string(), path: z.string() }))
    .mutation(({ input }) => {
      SourceModel.update(input.id, input.name, input.path);
    }),

  deleteSource: t.procedure.input(z.number()).mutation(({ input }) => {
    SourceModel.delete(input);
  }),

  // 移动规则管理
  listMoveRules: t.procedure.query(() => {
  ...
  deleteMoveRule: t.procedure.input(z.number()).mutation(({ input }) => {
  ...
  startScan: t.procedure
    .input(z.object({ fullScan: z.boolean().optional().default(false) }))
    .mutation(({ input }) => {
      const scanService = ScanService.getInstance();
      scanService.startScan(input.fullScan);
    }),

  stopScan: t.procedure.mutation(() => {
    const scanService = ScanService.getInstance();
    scanService.stopScan();
  }),

  getScanStatus: t.procedure.query(() => {
    const scanService = ScanService.getInstance();
    return scanService.getStatus();
  }),

  // 文件移动管理
  startMove: t.procedure.mutation(() => {
    const moveService = MoveService.getInstance();
    moveService.startMove();
  }),

  stopMove: t.procedure.mutation(() => {
    const moveService = MoveService.getInstance();
    moveService.stopMove();
  }),

  getMoveStatus: t.procedure.query(() => {
    const moveService = MoveService.getInstance();
