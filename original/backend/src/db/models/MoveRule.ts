...
export const MoveRuleModel = {
  list: (): MoveRule[] => {
    const db = getDb();
    return db.prepare('SELECT * FROM move_rule').all() as MoveRule[];
  },
  ...
  delete: (id: number): void => {
    const db = getDb();
    db.prepare('DELETE FROM move_rule WHERE id = ?').run(id);
  },

  enable: (id: number, enabled: boolean): void => {
    const db = getDb();
    db.prepare('UPDATE move_rule SET enabled = ? WHERE id = ?').run(enabled ? 1 : 0, id);
  },
};
