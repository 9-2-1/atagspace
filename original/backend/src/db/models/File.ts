...
export const FileModel = {
  setTags: (id: number, tags: string): void => {
    const db = getDb();
    db.prepare('UPDATE file SET tags = ? WHERE id = ?').run(tags, id);
  },

  getTags: (id: number): string => {
    const db = getDb();
    return db.prepare('SELECT tags FROM file WHERE id = ?').get(id) as string;
  },
  ...
  list: (path: string): File[] => {
    ...
    return db
      .prepare('SELECT * FROM file WHERE path = ? AND deltime IS NULL ORDER BY name')
      .all(path) as File[];
  },
  ...
  listRecurse: (path: string): File[] => {
    ...
    if (path === '/') {
      return db
        .prepare('SELECT * FROM file WHERE deltime IS NULL ORDER BY path, name')
        .all() as File[];
    }
    const slash = '/';
  ...
  markAllDelete: (): void => {
    const db = getDb();
    db.prepare('UPDATE file SET deltime = ? WHERE deltime IS NULL').run(Date.now());
  },
  ...
  exists: (path: string, name: string): boolean => {
    const db = getDb();
    const result = db.prepare('SELECT 1 FROM file WHERE path = ? AND name = ?').get(path, name);
    return result !== undefined;
  },

  getPathName: (path: string, name: string): File | null => {
    const db = getDb();
    const result = db.prepare('SELECT * FROM file WHERE path = ? AND name = ?').get(path, name);
  ...
  getDevIno: (dev: number, ino: number): File | null => {
    ...
    const result = db.prepare('SELECT * FROM file WHERE dev = ? AND ino = ?').get(dev, ino);
    return result as File | null;
  },

  updatePathName: (id: number, newPath: string | null, newName: string | null): void => {
    const db = getDb();
    const oldFile = db.prepare('SELECT path, name FROM file WHERE id = ?').get(id) as {
      path: string;
      name: string;
    };
    ...
    db.prepare('UPDATE file SET name = ?, path = ? WHERE id = ?').run(finalName, finalPath, id);
  ...
  purgeDeleted: (time: number): void => {
    const db = getDb();
    db.prepare('DELETE FROM file WHERE deltime IS NOT NULL AND deltime < ?').run(time);
  },
};
