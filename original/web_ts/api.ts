async function _api(path: string, param: any) {
  return await (
    await fetch(path, { method: "POST", body: JSON.stringify(param) })
  ).json();
}
...
let api = {
  ...
  remove_category: (param: { name: string }) =>
    _api("/remove_category", param) as Promise<null>,
  set_tags: (param: { tags: Array<string>; cate: string }) =>
    _api("/set_tags", param) as Promise<null>,
  remove_tags: (param: { tags: Array<string> }) =>
    _api("/remove_tags", param) as Promise<null>,
  ...
  tag_file_change: (param: {
    ids: Array<number>;
    adds: Array<string>;
    removes: Array<string>;
  }) => _api("/tag_file_change", param) as Promise<null>,
  open_content: (path: string) =>
    _api("/open_content", { path }) as Promise<null>,
};
